...
make_kud <- function(analyse, zoom_levels, comportement, GPS_sampled, data_generated_path, resolution_ZOOM, couleur) {
  ...
  map <- tm_scalebar() +
    ...
    tm_shape(data_95) + tm_polygons(border.col = "white", col = couleur, alpha = 0.5, legend.alpha = 1) +
...
ggsave(paste0(atlas_path, "/emission_plot.png"),
  plot = emission_plot, width = 12, height = 8, dpi = 300
)
...
area_hr_plot <- grid.arrange(hr_plot, hist_danslareserve,
  ncol = 2,
  heights = c(4, 1), widths = c(3.5, 1)
)

# Sauvegarde du NULL# Sauvegarde du graphique
ggsave(paste0(atlas_path, "/area_hr_plot.png"),
  plot = area_hr_plot, width = 12, height = 10, dpi = 300
)
...
param <- "month_label"
comportement <- "foraging"
couleur <- generate_color_gradient("#49B6FF", light_max = 1, dark_max = 1, n_total = 12)
scales::show_col(couleur)
