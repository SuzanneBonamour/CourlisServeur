...
all_gps <- all_gps %>%
  dplyr::select(all_of(colonnes_utiles)) %>%
  dplyr::rename(!!!noms_colonnes)
...
# pour récupérer les sex et gare correctement
# pour récupérer les sex et gare correctement
# pour récupérer les sex et gare correctement
# pour récupérer les sex et gare correctement
# pour récupérer les sex et gare correctement
...
# library(writexl)
# write_xlsx(donnees_2, path = "COURLIS_donnees_manquantes_sex_age.xlsx")
...
# /!\ /!\ /!\ data from GWEN sept 2025 /!\ /!\ /!\
# /!\ /!\ /!\ data from GWEN sept 2025 /!\ /!\ /!\ -----------------------------
# /!\ /!\ /!\ data from GWEN sept 2025 /!\ /!\ /!\

XY <- readRDS(paste0(data_generated_path_serveur, "/TracksCurlew/OutputData/FinalTracks/XY_tidebehaviorfinal.rds"))
...
# Nb ind
length(unique(XY$individual_local_identifier_origin))
...
behaviour_24h_BOX_1000_56_sex <- left_join(behaviour_24h_BOX_1000_56, sex_2, by = "ID")
...
GPS_clean <- behaviour_24h_BOX_1000_56_sex_age_breche %>%
  dplyr::rename(
    ...
    behavior = behavior,
    sex = sex, year_baguage = year_baguage, age_baguage = age_baguage,
    age = age, tides_high_type = tide, breche = breche
  )
...
verif_tz(all_trip_stationary_sf, "DateTime")
all_trip_stationary_sf$DateTime <- as.POSIXct(all_trip_stationary_sf$DateTime, tz = "UTC")
verif_tz(all_trip_stationary_sf, "DateTime")
verif_crs(all_trip_stationary_sf)
...
behaviour_24h_BOX_1000_56 <- behaviour_jour_nuit %>%
  ...
  filter(nb_point >= 100, nb_days >= 7) # 1000 & 56
