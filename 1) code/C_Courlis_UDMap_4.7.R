...
options(scipen = 999)
 ...
 ## functions -----------------------------------------------------------------
  ...
  estimate_kud <- function(analyse, zoom_level, comportement, GPS, data_generated_path, resolution_ZOOM) {
    ...
    sigma_y <- sd(GPS_coords.behavior[,2]) 
    nb <- nrow(GPS.behavior)
    h.silverman_x <- 1.06 * sigma_x * nb^(-1/5) / 4
    h.silverman_y <- 1.06 * sigma_y * nb^(-1/5) / 4
  ...
  results_kud = NULL
  nb_kud = NULL
  analyse <- "roosting_95_50_test_make"
  comportement <- "roosting"
  couleur = couleur_roosting
  
  make_kud <- function(analyse, zoom_levels, comportement, GPS, data_generated_path, resolution_ZOOM, couleur) {
    ...
    # in ZOOM
    library(sf)
    ZOOM_shape <- st_read(paste0(data_generated_path,"ZOOM_",zoom_levels,".gpkg"))
    ZOOM_shape <- st_transform(ZOOM_shape, crs = 4326)
    GPS.ZOOM <- st_intersection(GPS, ZOOM_shape) 
    ...
    # raster/grid
    grid <- st_read(paste0(data_generated_path, "grid_ZOOM_",zoom_levels,".gpkg"))
    raster <- rast(grid, resolution = resolution_ZOOM, crs="EPSG:2154")
    spatRaster <- project(raster, crs_utm)
    rasterLayer <- raster(spatRaster)
    spatialPixels <- as(rasterLayer, "SpatialPixels") 
    ...
    if (is.null(zoom_obj) || nrow(zoom_obj) == 0) {
      stop(paste("zoom_obj est vide ou NULL pour", zoom_levels))
    }
      
    bbox <- st_bbox(zoom_obj)
    if (any(is.na(bbox))) {
      stop(paste("bbox contient des NA pour", zoom_levels))
    }
    ...
    nb_kud <- nb_ind_point_dt
    # results_kud <- get(paste0("results_kud.", analyse))
    results_kud <- results_kud
    
    stats_row <- nb_kud[nb_kud$zoom == zoom_levels, ]
    nb_ind <- length(stats_row$ID)
    nb_point <- sum(stats_row$n)
    info_text <- paste0(nb_point, " points / ", nb_ind, " individus")
    point_text_info <- st_sfc(st_point(c(bbox["xmin"] + 1000, bbox["ymax"] - 1000)), crs = st_crs(zoom_obj))
    ...
    data_95 <- results_kud[results_kud$ZOOM == zoom_levels & results_kud$level == 95, ]
    data_90 <- results_kud[results_kud$ZOOM == zoom_levels & results_kud$level == 90, ]
    data_50 <- results_kud[results_kud$ZOOM == zoom_levels & results_kud$level == 50, ]
    
    if (nrow(data_95) == 0 || nrow(data_50) == 0) {
      stop(paste("Aucune donnée 95 ou 50 pour", zoom_levels))
    }
    ...
    map <- tm_scalebar() +
      ...
      tm_polygons(border.col = "white", col = couleur, fill_alpha = 0.3) +
      tm_shape(data_90) +
      tm_polygons(border.col = "white", col = couleur, fill_alpha = 0.5) +
      tm_shape(data_50) +
      tm_polygons(border.col = "white", col = couleur, fill_alpha = 0.7) +
      tm_shape(zoom_obj) +
      tm_borders(col = "#575757", lty = "dotted", lwd = 3) +
      tm_shape(label_point) +
      tm_text("label", col = "#575757", size = 3, options = opt_tm_text(just = c("left", "top"))) +
  ...
  make_kud <- function(analyse, zoom_levels, comportement, GPS, data_generated_path, resolution_ZOOM, couleur) {
    ...
    ### 4. KernelUD ###
    kud <- kernelUD(as_Spatial(GPS_spa.behavior), grid = spatialPixels, h = h)
    ...
    map <- tm_basemap(c("OpenStreetMap", "Esri.WorldImagery", "CartoDB.Positron")) +
      ...
      tm_shape(data_50) + tm_polygons(border.col = "white", col = couleur, alpha = 0.95) +
      ...
      tm_credits(info_text, position = c("left", "bottom"), size = 1,
...
test_fonction <- make_kud(analyse, zoom_levels, comportement, GPS, data_generated_path, resolution_ZOOM, couleur)
...
plan(multisession)  # utilise multicore sur Linux/macOS si souhaité

# Liste des zoom_levels
liste_zoom <- c("AA", "CC")  # adapte selon ton besoin
...
maps_list.roosting <- future_map(
  ...
  .options = furrr_options(
    seed = TRUE,
    packages = c("raster", "sf", "dplyr", "adehabitatHR", "terra", "tmap")
  )
)

# Revenir à l'exécution séquentielle si besoin
plan(sequential)
...
maps_list.roosting <- future_map(
    ...
    .options = furrr_options(
      ...
      globals = list(
        ...
        # results_kud.roosting_95_50_test_make = results_kud.roosting_95_50_test_make,
        # nb.roosting_95_50_test_make = nb.roosting_95_50_test_make,
        terre_mer = terre_mer,
...
valid_zoom <- zoom_levels[
  file.exists(paste0(data_generated_path, "ZOOM_", zoom_levels, ".gpkg"))
]
...
col_sex_age_en <- c("adult female" = "purple", "juvenile female" = "lightpink",
                 "adult male" = "darkgreen", "juvenile male" = "lightgreen",
                 "unknown" = "grey40")

emission_plot_talk <- ggplot(emission_dt_1_talk, aes(x = date, y = ID, 
                                           color = sex_age_en)) +
  geom_point(size = 2, shape = 15) +
  scale_color_manual(values = col_sex_age_en) +
  theme_classic() +
  scale_x_date(date_breaks = "10 month", date_labels = "%b %Y") +
  # theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1)) +
  labs(title = "",
       x = "Recording periods", 
       y = "Individual", 
       color = "Sex & Age") ; emission_plot_talk

# save plot
ggsave(paste0(atlas_path, "/emission_plot_talk.png"), 
       plot = emission_plot_talk, width = 12, height = 7, dpi = 1000)
...
var.test(paired_centroids_age_dt_2$distance_m[paired_centroids_age_dt_2$age == "adulte"], 
...
distance_roost_forag_plot_talk <- ggplot(paired_centroids_mean_dt, 
                                    aes(x = reorder(ID, mean_dist), y = mean_dist)) + 
  geom_hline(yintercept=mean_dist, color = "black") +
  geom_hline(yintercept=mean_dist + sd_dist, linetype="longdash", color = "grey") +
  geom_hline(yintercept=mean_dist - sd_dist, linetype="longdash", color = "grey") +
...
roosting and foragin areas (m)", fill="") ; distance_roost_forag_plot
...
distance_roost_forag_sex_plot <- ggplot(dt_distance_talk, 
...
roosting and foragin areas (m)", fill="") ; distance_roost_forag_sex_plot

my_comparisons <- list( c("adult", "juvenile"))

distance_roost_forag_age_plot <- ggplot(dt_distance_talk, 
                                        aes(x = age_en, y = distance_m, fill = age_en)) + 
  geom_boxplot(outlier.colour = "grey", outlier.shape = 1) +
  scale_fill_manual(values = c("adult" = "#D47545", "juvenile" = "#D2AB99"),
                    name = "Age") +
  geom_jitter(shape = 21, size = 0.5, color = "white", alpha = 0.5, fill = "black", width = 0.3) +
  stat_summary(fun.ymin = function(x) mean(x) - sd(x),
               fun.ymax = function(x) mean(x) + sd(x), geom="linerange", size=1, color="black") + 
...
roosting and foragin areas (m)", fill="") ; distance_roost_forag_age_plot
...
distance_roost_forag_tides_high_type_plot <- ggplot(dt_distance_talk_tide, 
...
roosting and foragin areas (m)", fill="") ; distance_roost_forag_tides_high_type_plot
...
zone_critique_hotspot_map_final_1 <- tm_basemap(c("OpenStreetMap", "Esri.WorldImagery", "CartoDB.Positron")) +
  ...
  tm_shape(site_baguage) +
  tm_text("icone", size = 1.5) ; zone_critique_hotspot_map_final_1
...
zone_critique_hotspot_map_final_2 <- tm_basemap(c("OpenStreetMap", "Esri.WorldImagery", "CartoDB.Positron")) +
  ...
  tm_shape(tonnes_zones_grouped_clean) +
  tm_basemap(c("OpenStreetMap", "Esri.WorldImagery", "CartoDB.Positron")) + 
  tm_polygons(fill = "#D64045", 
              style = "cont", alpha = 0.5,
              title = "Nb superposées") +
  tm_shape(tonnes) +
  tm_dots(fill = "black") ; zone_critique_hotspot_map_final_2
