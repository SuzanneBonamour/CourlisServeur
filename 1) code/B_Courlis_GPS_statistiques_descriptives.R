...
# Mise en forme desas.Date.default()# Mise en forme des données des points de suivi
# point_no_gap <- point_no_gap %>%
#   arrange(date_UTC) %>%
...
for (i in unique(tides$t)) {
  ...
  for (n in unique(dt_i_low$n)) {
    ...
    time_i_n <- dt_i_low$date_UTC[dt_i_low$n == n] # time
  ...
  for (n in unique(dt_i_high$n)) {
    ...
    time_i_n <- dt_i_high$date_UTC[dt_i_high$n == n]
    ...
    # all_info_high <- point_no_gap %>%
    #   mutate(behavior = case_when(between(date_UTC, roosting_period[1], roosting_period[2]) ~ "roosting")) %>%
    #   filter(!is.na(behavior)) %>%
    #   # dplyr::select(id, date, behavior, pkey) %>%
