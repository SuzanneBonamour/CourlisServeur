...
behaviour_24h_BOX_1000_56 <- behaviour_jour_nuit %>%
  ...
  filter(nb_point >= 1000, nb_days >= 56) # 1000 & 56
...
DATA_LIMI <- read_excel(file.path(data_path_serveur, "Age_Sex/sexe_age_dt_v2.xlsx"))
...
age_data <- DATA_LIMI %>% 
  select(ID, age) %>% 
  mutate(age_baguage = case_when(age %in% c("1A", "2A") ~ "JUV", 
                                 TRUE ~ "AD"))

table(age_data$age)
table(age_data$age_baguage)

year_baguage_dt <- read_excel(file.path(data_path_serveur, "Age_Sex/DATA_LIMI.xlsx"))

year_baguage_dt_2 <- year_baguage_dt %>%
  filter(ACTION == "B") %>%
  dplyr::select(BAGUE, Year) %>% 
  rename(ID = BAGUE, year_baguage = Year)

age_data_2 <- age_data %>% 
  left_join(year_baguage_dt_2)
