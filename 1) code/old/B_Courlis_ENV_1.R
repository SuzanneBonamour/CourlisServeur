...
# rebathy# restriction bathy à la zone focale autour de la reserve
...
# grid 100x100 ---
grid <- st_read(paste0(data_generated_path, "grid_100x100.gpkg"))
crs(grid)
...
# carrés <- st_read("carrés.shp")
# habitats <- st_read("habitats.shp")

# Faire l'intersection entre les carrés et les habitats pour obtenir les zones communes
grid_2154 <- st_transform(grid, crs = 2154)
seahab_2154 <- st_transform(seahab, crs = 2154)

grid_2154$id <- c(1:length(grid_2154$geom))

intersections <- st_intersection(grid_2154, seahab_2154)

# Calculer la superficie de chaque zone d'habitat dans l'intersection
intersections <- intersections %>%
  mutate(superficie = st_area(geom))  # Calculer la superficie pour chaque zone d'habitat

# intersections$id <- c(1:length(intersections$gid))

# Trouver l'habitat avec la plus grande superficie pour chaque carré
habitat_max_superficie <- intersections %>%
  group_by(id_carré = intersections$id) %>%  # Assurez-vous que vous avez un identifiant pour les carrés
  arrange(desc(superficie)) %>%
  slice(1) %>%  # Sélectionner l'habitat avec la plus grande superficie
  ungroup() %>%
  dplyr::select(id_carré, habitat_max_superficie = annexi, superficie)  # Remplacer 'nom_habitat' par le nom réel du champ

# habitat_max_superficie_tt <- unique(habitat_max_superficie$id_carré)

# Joindre l'habitat avec la plus grande superficie aux carrés
# carrés_with_max_habitat <- left_join(grid, habitat_max_superficie, by = "id_carré")
crs(grid_2154)
crs(habitat_max_superficie)

grid_2154$id_carré <- grid_2154$id

grid_2154 <- st_join(grid_2154, habitat_max_superficie, by = "id_carré")

grid_2154_noNA <- grid_2154[!is.na(grid_2154$habitat_max_superficie),]

grid_2154_noNA_unique <- grid_2154_noNA[unique(grid_2154_noNA$id_carré),]

# Visualiser le résultat

plot(grid)
plot(grid["habitat_max_superficie"])


tt <- unique(habitat_max_superficie$id_carré)

tt <- tm_scale_bar() +
  tm_shape(RMO) +
  tm_polygons(col = "white", alpha = 0.3) +
  tm_shape(habitat_max_superficie) +
  tm_polygons(col = "habitat_max_superficie") ; tt

tt2 <- tm_scale_bar() +
  tm_shape(RMO) +
  tm_polygons(col = "white", alpha = 0.3) +
  tm_shape(grid_2154) +
  tm_polygons(col = "habitat_max_superficie") ; tt2
...
# Faire l'intersection entre les carrés et les habitats pour obtenir les zones communes
intersections <- st_intersection(carrés, habitats)

# Calculer la superficie de chaque zone d'habitat dans l'intersection
intersections <- intersections %>%
  mutate(superficie = st_area(geometry))  # Calculer la superficie pour chaque zone d'habitat
...
habitat_max_superficie <- intersections %>%
  ...
  slice(1) %>%  # Sélectionner l'habitat avec la plus grande superficie
  ungroup() %>%
  select(id_carré, habitat_max_superficie = nom_habitat)  # Remplacer 'nom_habitat' par le nom réel du champ

# Ajouter la nouvelle colonne "habitat_max_superficie" dans l'objet carrés
carrés <- left_join(carrés, habitat_max_superficie, by = c("id_carré" = "id_carré"))

# Visualiser le résultat
plot(carrés["habitat_max_superficie"])
...
carrés_with_max_habitat_unique <- unique(carrés_with_max_habitat["habitat_max_superficie"])

plot(unique(carrés_with_max_habitat["habitat_max_superficie"]))
...
tt <- tm_scale_bar() +
  tm_shape(RMO) +
  tm_polygons(col = "white", alpha = 0.3) +
  tm_shape(carrés_with_max_habitat) +
  tm_polygons() ; tt
...
# pour les aires
# st_intersects
lengths(st_intersects(carre, habitat)) # pour savoir ou ya 1 seule habitat > à garder de coter pour gagner du temps
# pour les carré ou y'en a plusieurs des habitats 
st_area(habitat)
st_intersection(habitat, carre)
...
r <- rast(nrows=50, ncols=50, ext=c(-10, 10, -10, 10), nlyr=2,
          vals=0.5, names=c("Jan","Feb")) * c(1,3)
...
extract(r, seahab[1:3,], fun=mean, na.rm=TRUE)

r <- rast(grid, ncols=100, nrows=100)

temp <- rasterize(grid, r)

plot(r)
plot(temp)

raster1 <- raster(nrow = 1800, ncol = 4320)

seahab_rast <- rasterize(x = seahab$geometry, y = raster1, field = seahab$annexi) 
...
seahab_rast <- fasterize(seahab$geometry)

library(exactextractr)
test<-grid 
crs(grid) 
crs(bathy, crs = 2154) <- bathy
bathy <- crs(2154) 
crs(bathy) <- "2154"

bathbathybathy_2154 <- st_transform(bathy, crs= 2154)
test_corine <- corine 
test_corine$ingrid <- exact_extract(corine, grid, "mode")
